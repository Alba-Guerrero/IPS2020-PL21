...
public class VentanaMedico extends JDialog {
	 ...
	 * @param string 
	 */
	public VentanaMedico(String string) {
		codmedico=string;
		setTitle("Ventana M\u00E9dico");
	...
	private JButton getBtnVerCitas() {
		if (btnVerCitas == null) {
			btnVerCitas = new JButton("Ver calendario de citas");
	...
	protected void asignarCitas() throws SQLException {
		VerCitasMedico vca = new VerCitasMedico();
		
		vca.setLocationRelativeTo(null);
		vca.setResizable(true);
		vca.setModal(true); // hasta que no se cierre una ventana no se puede abrir otra
		vca.setVisible(true);
	...
	protected void verCitas() {
		VentanaMedicoCita vmc =new VentanaMedicoCita(codmedico);
